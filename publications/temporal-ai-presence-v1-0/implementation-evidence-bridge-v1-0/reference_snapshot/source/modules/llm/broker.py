...
def _http_json(url: str, payload: Dict[str, Any], headers: Dict[str, str], timeout: int = TIMEOUT) -> Dict[str, Any]:
    ...
    if host in _DIRECT_CLOUD_AI_HOSTS:
        return {
            "ok": False,
            "error": "cloud_ai_requires_oracle_route",
            "route": "tap-cloud-openai-oracle",
            "network_attempted": False,
        }
    if not allow_network(url):
        denied = deny_payload(url, target="llm_broker_transport")
        denied["network_attempted"] = False
        return denied
