...
from modules.net_guard import allow_network, is_local_url
...
def _call_llm(model: str, messages: List[Dict[str, str]], *, temperature: float) -> Tuple[str, str]:
    if not is_local_url(LMSTUDIO_URL) or not allow_network(LMSTUDIO_URL):
        return "", "local_model_route_denied"
