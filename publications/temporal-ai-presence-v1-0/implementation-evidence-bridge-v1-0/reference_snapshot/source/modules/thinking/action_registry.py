...
def _record_conflict_safely(**kwargs: Any) -> None:
    # Ledger failure must never change the already-computed policy decision.
    try:
...
def _record_gate_conflict(kind: str, args: Dict[str, Any], vctx: Any, decision: Any, *, source: str) -> None:
    ...
    if source == "volition_gate.observe":
        # Slot A still allows execution; would-deny is only recorded for review.
...
def _append_oracle_deny_volition(payload: Dict[str, Any], rep: Dict[str, Any]) -> None:
    ...
    _record_conflict_safely(
        ...
        step_index=step_index,
        args_digest=args_digest,
        prompt_digest=hashlib.sha256(str(payload.get("prompt") or "").encode("utf-8")).hexdigest()
